...
sed -i 's/tryflag CFLAGS_AUTO -Qunused-arguments//' configure

#########
# Configure
$PKGDIR/B-configure-1 --prefix=/opt/dev-x86_32 --bindir=/usr/bin --disable-shared || exit 1
...
cd $DST || exit 1
rm -rf lib etc
rm -f opt/dev-x86_32/lib/libc.so
[ -d bin ] && strip bin/*
