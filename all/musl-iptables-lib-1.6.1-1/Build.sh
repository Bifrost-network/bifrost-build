...
VER=1.6.1
SRCVER=iptables-$VER
PKG=musl-iptables-lib-$VER-1 # with build version
...
pkg_install musl-libnetfilter_conntrack-1.0.6-1 || exit 2
pkg_install musl-libnfnetlink-1.0.1-1 || exit 2
pkg_install musl-libnftnl-1.0.7-1 || exit 2
...
#########
# Configure
LIBS="-lnetfilter_conntrack -lmnl -lnfnetlink" B-configure-3 --prefix=/opt/musl --enable-libipq --enable-bpf-compiler || exit 1
...
# Clean up
cd $DST || exit 1
rm -rf opt/musl/share opt/musl/sbin opt/musl/bin
