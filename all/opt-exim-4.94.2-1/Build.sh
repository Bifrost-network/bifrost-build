...
pkg_install perl-5.10.1-4 || exit 2
...
cd $BUILDDIR || exit 1
libtool_fix-1
#patch -p0 < $PKGDIR/gnuc.pat  || exit 1
...
echo  "CFLAGS=-static -Os -march=i586 -std=c99 -D_XOPEN_SOURCE -D_GNU_SOURCE -D_BSD_SOURCE"   >>  Local/Makefile
...
sed -i 's/SUPPORT_DANE=yes/#SUPPORT_DANE=no/' Local/Makefile

make configure
patch -p0 < $PKGDIR/NS.pat  || exit 1
