...
pkg_install bitsperlong-1 || exit 2
...
dopatch -p0 $PKGDIR/i40e_rss.pat || exit 1
...
# Backported
dopatch -p1 $PKGDIR/vlan.pat || exit 1

# Increase nr of percpu pages
sed -i 's,batch = zone->managed_pages / 1024,batch = zone->managed_pages / 256,' mm/page_alloc.c
...
cp .config $DSTS/boot/$PKG-bifrost.config
